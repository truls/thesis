...
\item[Principle of least astonishment.]
  % Constructs in SME resembling constructs
...
%however, we deemed this to be an overly restrictive requirement.

The module system of SMEIL contributes towards the goal of creating a library of
reusable components for SME.
%\todo{Did we mention that this was a goal?}
...
% a network is also allowed in the declarative part of a process. \todo{Why is this?}
...
more accurate type unification (see \Cref{sec:typesys}). Due to this, the
...
described in \Cref{sec:typing}. Currently, the given range is only used to
...
Enumerations are a useful way of specifying closely associated named
...
%The perhaps most interesting of the declarations described here are buses.
...
co-simulation (\Cref{sec:cosim}) or through the top-level entity (see the next
section) of the generated VHDL code (\Cref{sec:codegen}).
...
referencing buses declared within the entity instance. \Cref{fig:comms} shows
...
of a particular entity.  \Cref{fig:anonproc} shows two networks utilizing
...
%\todo{Move to after instance intro}
...
the connections of a network such as {\sc addone} (seen in \Cref{sec:smeilex}) is
...
structure. For example, our MD5 example shown in \Cref{sec:md5} was derived from
...
\begin{grammar}
  ...
  \alt <string-char> (Any printable character)
\end{grammar}
...
fallthroughs.
% , although in order for this to be done, we need to find a way for
...
of non-boolean operators (see \Cref{sec:typesys}). Thus, logical operators
...
\Cref{sec:typing}) will provide some assurance that an overflow will not
...
% Still, the type system of SMEIL is an obvious target
% for future improvements\todo{fix}.
...
% declarative part of a process \todo{finish or remove}
...
%   \Cref{sec:instance}). References to a bus from the process where it is
...
\begin{figure}
  ...
  \resizebox{.7\linewidth}{!}{
    \begin{tikzpicture}[font=\tiny,
      ...
      \draw[-{Latex[scale=1.6]}] (id) edge [bend right=30] node [above] {{\ttfamily addout}} (add);
...
% Before closing this chapter, we show an example of a small, but complete
...
% in \Cref{fig:addone} to give a feel of the language and illustrate the basic
...
% source code for the example is shown in \Cref{fig:addone.sme}.

Before closing this chapter, we show an example of a small, but complete SME
...
\Cref{fig:addone}, consists of two processes and two buses. One process, named
...
parameter. The source code for the example is shown in \Cref{fig:addone.sme}.
