...
use-pattern based design process \todo{finish or remove} In the design of the
...
programmer. The handling of modules in SMEIL is described further in \todo{Where
  is this?}.
...
be static at compile time. \todo{Why is this?}
...
\Cref{sec:typesys}) as it allows for more accurate type unification. In fact,
...
use is described in \Cref{sec:typing}. Currently, the given range is simply used
...
within the instantiated entity. \Cref{fig:comms} shows three different ways that
...
\paragraph{Loops.}\texttt{for}-loops have, unlike in C, a slightly simpler syntax. For example,
the following,
\begin{lstlisting}[language=smeil]
for i = 1 to 10 {
  trace("{}", i);
...
are also supported (more on those in \Cref{sec:typing}) and are denoted simply
...
% For report: How this is implemented
